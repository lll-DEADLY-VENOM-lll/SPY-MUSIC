...
# Progress Bar Helper
def generate_bar(played_sec, duration_sec):
    try:
...
def stream_markup_timer(_, videoid, chat_id, played, dur):
    bar = generate_bar(time_to_seconds(played), time_to_seconds(dur))
    buttons = [
        [
...
def stream_markup(_, videoid, chat_id):
    buttons = [
        ...
        [InlineKeyboardButton(text="๏ ᴀʟʟ ғᴇᴀᴛᴜʀᴇs ๏", callback_data=f"Pages Forw|0|{videoid}|{chat_id}")],
    ]
    return buttons

def track_markup(_, videoid, user_id, channel, fplay):
    # Note: track_markup usually doesn't have chat_id in the caller. 
    # Using videoid as a placeholder if chat_id is needed for buttons.
    buttons = [
        [InlineKeyboardButton(text=_["S_B_5"], url=f"https://t.me/{app.username}?startgroup=true")],
        [
...
def telegram_markup(_, chat_id):
    buttons = [[InlineKeyboardButton(text=_["CLOSEMENU_BUTTON"], callback_data="close")]]
    return buttons
